...
class MechashipImuMagHardIronNode(Node):
    def __init__(self):
        ...
        self.create_subscription(
            MagneticField, "imu/mag", self._mag_sub_callback, qos_profile_sensor_data
        )
    ...
    def _mag_data_monitor(self):
        if self._is_first_data == False:
            self.get_logger().warning(
                'Waitting for Magnetic Field sensor data from "imu/mag" topic...'
            )
        else:
    ...
    def _mag_sub_callback(self, data: MagneticField):
        if self._is_first_data == False:
            ...
            self.get_logger().info(
                "First Magnetic Field sensor data has been received!"
            )
    ...
    try:
        ...
        if len(mag_arr) == 0:
            ...
            path_str = os.path.join(
                os.path.expanduser("~"), "mag_arr_" + time_str + ".csv"
            )
            ...
            # bias 계산
            bias_x, bias_y, bias_z = calculate_magnetometer_bias(
                _x_mag_arr, _y_mag_arr, _z_mag_arr
            )
